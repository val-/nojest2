...
Другое направление угроз при разработке прототипа происходит из устанавливаемых библиотек в виде NPM пакетов. Это направление появилось недавно. Как правило, разработчики популярных библиотек придерживаются нравственных принципов свободного и открытого программного обеспечения. Однако существует большой список скомпрометированных NPM пакетов toxic-repos. [4.6]
Помимо непосредственных зависимостей проекта, уязвимости могут содержать зависимости используемых библиотек.
Вредоносный код зараженного NPM пакета может перезатирать файлы на компьютере с проектом, добавлять контент от злоумышленников в разрабатываемое приложение.

Для минимизации рисков по этому направлению предлагается провести мероприятия:
* анализ исходных кодов подключаемых библиотек
* проверка зависимостей и зависимостей зависимостей из package-lock.json по списку опасных пакетов из toxic-repos
