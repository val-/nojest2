...
Логическая модель базы данных необходима для понимания, как циркулируют данные в информационной системе. Эта модель строится проектировщиком, а используется разработчиком. Существует несколько нотаций для построения логической модели данных. Наиболее распростанены нотации Питера Чена, Crows Foot, IDEF1X. В данной работе для визуализации логической модели используется нотация Crows Foot т.к. она проста в построении и достаточно наглядна. В этой нотации сущности показваются в виде прямоугольников, которые содержат заголовок с названием сущности и список полей ниже. Отношения между сущностями показываются в виде линий, соединяющих прямоугольники. Далее будут перечислены логические сущности и их описания.

**Пользователь** (user)
Экземпляр этой сущности преставляет собой учётную запись пользователя. Содержит профиль пользователя и служебные поля (хэш пароля и токен подтверждения электронной почты).

**Проект** (ject)
Проект программного обеспечения является единицей репрезентации результа работы компании. Содержит название, описание и статус.
