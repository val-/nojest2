...
Текстовое представление плана запроса может быть преобразовано в графический отчёт, который призван показать разработчику возможные пути оптимизации времени выполнения или потребляемых ресурсов.

**Применение алгоритмов расчёта дистанции редактирования для реализации подсказок глобального текстового поиска.**
Существует множество алгоритмов поиска подстроки. Примитивный алгоритм поиска (рисунок 2.18) состоит из вложенных циклов перебора массива текста (*haystack*) и символов искомой строки (*needle*), возвращает позицию первого включения подстроки поиска (*match*).

![image](./img/2_18.png)
Рисунок 2.18 Блок-схема примитивного алгоритма поиска подстроки.

Несмотря на простоту и очевидность этого решения, примитивный алгоритм показывает плохие результаты по времени выполнения.
$$ T < O(|needle| * |haystack|) $$

Большая часть удачных математических решений уже имеет качественную программную реализацию под свободной лицензией. Кроме того высокоуровневые языки программирования имеют собственную реализацию поиска подстроки. В случае если объём входных данных имеет ограничение (пусть и достаточно большое) целесообразно применять оптимизацию поиска через предварительное построение индексов подстрок и их сочетаний, лексем. Объём построенных индексов должен не превышать допустимые квоты оперативной памяти. Фактические ограничения по размеру базы данных будут определены для целевого оборудования опытным путём при нагрузочном тестировании. В качестве начальных условий будут выбраны объемы сопоставимые с размером базы данных уже существующей системы управления проектами. При необходимости прототип может быть доработан с целью повышения производительности поиска, реализация высоконагруженных модулей может быть переписана на низкоуровневом языке программирования.

При построении индекса следует определить совокупный алфавит, который будет использоваться в текстовых данных системы. Для прототипа предполагается, что текст может содержать цифры, символы русского и английского языка, специальные символы, доступные для ввода с клавиатуры (~!@#$%^&*(){}`"'№?-_=+<>,./), что определяет мощность алфавита равным 100. Так индекс триграмм (сочетаний из трёх букв) будет содержать 1 000 000 записей.

Для реализации подсказок в глобальном текстовом поиске потребуется применение алгоритмов нечёткого поиска. Нечёткий поиск – это поиск информации, при котором выполняется сопоставление информации заданному образцу поиска или близкому к этому образцу значению. Для работы нечёткого поиска требуется вспомогательный алгоритм, который должен определять метрику различий между схожими строками. Понятие дистанции редактирования было введено Владимиром Иосифовичем Левенштейном в 1965 году. Дистанция редактирования определяет разность между двумя строками, которая выражается численно, как минимальное количество односимвольных операций (вставки, удаления или замены), необходимых для превращения одной строки в другую. Значение этого расстояния определяется функцией *d* от множеств символов *a* и *b*. Функция *d* может быть записана в виде рекуррентной формулы.
...
d_{a, b}(i, j) = 
 \begin{cases}
   ...
   d_{a, b}(i, j - 1) + 1\\
   d_{a, b}(i - 1, j) + 1\\
   d_{a, b}(i - 1, j - 1) + 1_{(a_{i} != b_{j})}
   \end{cases}
    &\ i > 0 \ \ j > 0 \\
 \end{cases}
\end{equation*} $$

В данной формуле шаг по *i* означает удаление из первой строки, шаг по *j* — вставку в первую строку, а шаг по обоим индексам символизирует замену символа или отсутствие изменений.

Существует множество других метрик схожести строк, среди них полезно рассмотреть расстояние Дамерау — Левенштейна. Эта метрика отличается от дистанции редактирования Левенштейна добавлением операции перестановки соседних символов (транспозиции). 
...
Применение метрики Дамерау — Левенштейна целесообразно т.к. большая часть ошибок при наборе текста пользователем является транспозицией. Но алгоритм вычисления этого расстояния редактирования более сложный. В разрабатываемом прототипе будут реализованы обе метрики для оценки производительности их алгоритмов и сравнения релевантности поиска.

# 2.4 Техническое обеспечение ИС

Под техническим обеспечением понимается вся совокупность средств, позволяющих создать техническую основу для использования информационной системы. Это вычислительные машины, линии связи и другое оборудование обеспечивающее работу системы и документация на эти средства и технологические процессы.

Проектируемый в данной работе продукт должен обладать: высокой доступностью, так как поток обращений непрерывный; надежностью, так как потеря данных плохо сказывается на успехе компании; масштабируемостью, так как количество проектов и задач со временем будет увеличиваться. В связи с этим для информационной системы была выбрана многоуровневая архитектура.

Информационная система будет состоять из четырёх компонентов:

1. **Клиент приложения** – уровень представления, реализация которого функционирует в браузере пользователя. Графический интерфейс реализуется на веб-технологиях, состоит из дерева React компонентов. Для быстрого прототипирования применяется набор элементов Material UI.

2. **Сервер приложения** – уровень логики, на котором происходит
обработка данных, полученных с уровня представления. Сервер реализован в виде программы на Node JS. Для быстрого прототипирования используется веб-фреймворк Express. На данном уровне располагаются модули информационной системы, которые с учетом бизнес-логики производят обработку данных. Взаимодействие с уровнем данных
происходит с помощью SQL-запросов.

3. **Сервер базы данных** – уровень данных, предназначенный для хранения информации. Доступ к чтению и записи данных реализован средствами СУБД PostgreSQL.

4. **Облачное хранилище документов** – уровень специальных данных, предназначен для обеспечения работы с документами системы. Выделен в отдельный компонент т.к. реализован в виде независимой информационной системы, которая имеет открытую лицензию и обеспечивает взаимодействие с разрабатываемым прототипом по программному интерфейсу.


Клиент передает на сервер запросы на чтение или корректировку данных. Сервер реализует требуемые выборки или коррективы, отслеживая в любом процессе полноценность и нерушимость данных, а итоговый параметр кода возврата или набора записей передаёт на клиент приложения. Клиент-серверная архитектура позволяет лучшим образом разделять нагрузки между клиентом и сервером, что оказывает положительное влияние на систему в целом: скорость работы, скорость разработки и поддержки. Также доступен перенос базы данных с одной системы управления базами данных на другую и корректировка требуемых данных без обновления клиентского приложения.

Для работы прототипа информационной системы необходим аппаратный комплекс. Предполагается, что компания обеспечивает работу сотрудников на персональных компьютерах объединённых в локальную или виртуальную частную сеть. Одним из узлов этой сети должен быть выделенный сервер. Развёртывание выполняется путём создания docker образов и запуска их в контейнере на выделенном сервере (рисунок 2.19). Клиент приложения после сборки становится доступен браузерам пользователей через обращение к серверу.

Диаграмма развертывания (deployment diagram) - диаграмма, на которой представлены узлы выполнения программных компонентов реального времени, а также процессов и объектов. Данная диаграмма является формой физического представления программной системы. Прототип разработан таким образом, что для работы требуется минимальный набор оборудования, это сделано для ускорения процесса разработки. Однако позволяет применять более сложные схемы развёртывания в дальнейшем.

![image](./img/2_19.jpg)
Рисунок 2.19 Диаграмма развертывания с применением контейнеризации

Для выполнения нагрузочного тестирования на одной рабочей станции может быть запущено несколько экземпляров клиента приложения (рисунок 2.20). При этом следует учитывать, что узким местом по производительности может стать канал связи между клиентом и сервером приложения.

![image](./img/2_20.jpg)
Рисунок 2.20 Диаграмма развертывания для нагрузочного тестирования

Для промышленной эксплуатации может быть целесообразно распределить ресурсы нескольких компьютеров для разных компонентов системы. На рисунке 2.21 показан такой сценарий, кроме того прототип может быть запущен без применения контейнеризации, но это усложняет процесс настройки окружения.

![image](./img/2_21.jpg)
Рисунок 2.21 Диаграмма развертывания без контейнеризации


При опытной эксплуатации в качестве сервера будет использоваться персональный компьютер следующей конфигурации: AMD Ryzen 5 3500X 32Gb RAM. В качестве примера рабочей станции сотрудника: Intel Core i3 2310M 8Gb RAM. Выбор конфигурации оборудования не ограничивается этими вариантами т.к. реализация ИС выполнена на высокоуровневом языке программирования и не зависит от архитектуры компьютера. В случае запуска прототипа ИС на виртуальном выделенном сервере ему потребуется 4 потока выполнения 8 Gb оперативной памяти и канал связи стандарта Gigabit Ethernet для прохождения нагрузочного тестирования.
